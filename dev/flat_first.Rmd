...
rarefied_microbiome <- function(microbiome_matrix, 
                                threshold = 0.05){
  microbiome_matrix <- microbiome_matrix |> tidyverse::as_tibble() |> tidyverse::column_to_rownames("numero_individu")
  abundant_otus <- tidyverse::tibble(
    V1         = colnames(microbiome_matrix),
    prevalence = colMeans(microbiome_matrix > 0)
  ) |> rename(OTU = V1) |> filter(prevalence >= threshold) |> tidyverse::pull(OTU)
...
assign_taxa <- function(founder_object,
  ...
  if(sum(table(taxa_assign)[table(taxa_assign) > 10 & table(taxa_assign) < 25]) < round(length(taxa_assign)*taxa_g)){
    print(glue::glue("{sum(table(taxa_assign)[table(taxa_assign) > 10 & table(taxa_assign) < 25])} taxa available for genetic control sampling but {round(length(taxa_assign)*taxa_g)} asked. Change taxa_g parameter please."))
  }
...
gen_effect_calibration <- function(founder_object,
  ...
  #Genotype
  G <- MoBPS::get.geno(attr(founder_object,"population"),gen=1)
  ...
  if(plot){
    p <- out_data %>% tidyverse::ggplot(aes(x = as.factor(effect.size),y=Heritability, fill = as.factor(effect.size))) +
      tidyverse::geom_boxplot()+
      labs(title = glue::glue("Heritabilities distribution"),
           subtitle = glue::glue("({length(unique(out_data$Taxa))} taxa under genetic control)"),
    ...
    p2 <- out_data %>% tidyverse::ggplot(aes(x=Heritability,y=as.factor(effect.size),fill=as.factor(effect.size))) +
      tidyverse::geom_density_ridges(alpha=0.8)+
...
compute_beta_matrix_cluster <- function(n_b,
  ...
  if(is.null(beta_info)){
    beta_info <- tidyverse::tibble(
      cluster     = 1:nb_k,
      id_otu      = split(1:length(n_clust), n_clust)[-1],
      id_qtl_o    = tidyverse::map(cluster, \(x) {sample(1:n_g, n_qtl_o)})
    )
  }
...
compute_mean_microbiome <- function(microbiome, dir = F, n_ind = NULL, ao, mix.params){
  ...
  if(dir){
    ...
    dir_mean <- dirmult::rdirichlet(n_ind,as.numeric(mean_microbiome)*ao) %>% t()
...
select_individual <- function(phenotypes,
  ...
  if(selection){
   ...
   if(!selection_type %in% c("GB", "B", "G", "diversity", "div.GB")){
    ...
    F_id <- score[grep("^F",rownames(score)),] %>% sort(decreasing = T) %>% names() %>% stringr::str_extract("[0-9]+") %>% head(n_F)
    M_id <- score[grep("^M",rownames(score)),] %>% sort(decreasing = T) %>% names() %>% stringr::str_extract("[0-9]+") %>% head(n_M)
    ...
    #extract individuals ID
    F_id <- colnames(genotypes)[grep("^F",colnames(genotypes))] %>% stringr::str_extract("[0-9]+") %>% sample(n_F)
    M_id <- colnames(genotypes)[grep("^M",colnames(genotypes))] %>% stringr::str_extract("[0-9]+") %>% sample(n_M)
  }
...
holo_simu <- function(h2,
  ...
  colnames(microbiome_gen) <- MoBPS::get.geno(population, gen = 1) %>% colnames()
  ...
  #####################
  params_phenotypes <- calibrate_params_phenotypes(X0 = MoBPS::get.geno(population,gen = 1),
  ...
  phenotypes_founder <- compute_phenotypes(X = MoBPS::get.geno(population,gen = 1), 
  ...
  G0 <- list(microbiome = microbiome_gen |>  t() |> compositions::clrInv() |> t(), 
             genotypes = MoBPS::get.geno(population,gen = 1), 
  ...
  for(i in 1:n_gen){
    ...
    if(i==1){
      ...
      population <- MoBPS::breeding.diploid(population,
    ...
    microbiome_gen <- microbiome_gen |>  t() |> compositions::clrInv() |> t()
    current_mother_microbiomes <- sapply(MoBPS::get.pedigree(population, gen=i+1)[,"mother"],function(x){
      microbiome_gen[,x]
    })
    ...
    if(!is.null(env_gen)){
      if(env_gen[i]){
        microbiome_gen <- compute_current_microbiome(beta = beta,
                                                     current_genotypes = MoBPS::get.geno(population,gen = i+1),
    ...
    ######
    phenotypes <- compute_phenotypes(X = MoBPS::get.geno(population,gen = i+1), 
                                     B = microbiome_gen, #CLR abundances 
                                     Nqtl_y = Nqtl_y, 
                                     Notu_y=length(otu_list), 
                                     otu_list = otu_list,
    ...
    ID_selected <- select_individual(phenotypes = phenotypes,
                                     microbiomes = microbiome_gen |>  t() |> compositions::clrInv() |> t(),
                                     genotypes = MoBPS::get.geno(population,gen = 1+i),
                                     beta = beta,
                                     beta_otu = params_phenotypes$omega,
                                     selection = selection,
    ...
    #update output
    ###
    list_output[[glue("G{i}")]] <- list(microbiome = microbiome_gen |>  t() |> compositions::clrInv() |> t(),
                                        genotypes = MoBPS::get.geno(population,gen = 1+i),
                                        phenotypes = phenotypes,
                                        pedigree = MoBPS::get.pedigree(population,gen = i+1),
                                        mean_microbiome = mean_microbiome,
                                        noise_microbiome = attr(microbiome_gen,"noise_microbiome"),
                                        ID_selected = c(glue("F_{ID_selected$F_id}"),glue("M_{ID_selected$M_id}")),
                                        omega_beta_g = as.vector(params_phenotypes$omega %*% (beta[rowSums(beta) != 0, ] %*% MoBPS::get.geno(population,gen = 1+i))))
    if(verbose){
      end_g <- proc.time() - start_g
      print(glue::glue("Executed in {round(end_g[1] |> as.numeric(),3)} seconds"))
    }
  }
  return(list_output)
}
...
richness_from_abundances_gen <- function(microbiome_matrix, n_loop=10, plot=T){
  ...
  for(i in 1:n_loop){
    ...
    tmp_physeq <- phyloseq::phyloseq(otu_table(microbiome_matrix,taxa_are_rows = T))
    if(i==1){
      richness <- phyloseq::estimate_richness(tmp_physeq,measures = c("Observed","Shannon","InvSimpson"))
    }else{
      richness <- richness + phyloseq::estimate_richness(tmp_physeq,measures = c("Observed","Shannon","InvSimpson"))
    }
  }
...
get_phenotypes <- function(data) {
  data |> pluck("phenotypes") |> as.data.frame() |> tidyverse::as_tibble(rownames = "ID") 
}
