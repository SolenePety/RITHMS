...
#' The `microbiome` matrix contains taxa (in columns) accross individuals (in rows).
#' The `population` object contains genotypes, encoded as 0,1,2 and generated using the [MoBPS](https://pubmed.ncbi.nlm.nih.gov/32229505/) package.
#' 
...
generate_founder <- function(path = NULL,
  ...
  if(is.null(path)){
    ...
    if(file_type == "pedmap"){
      ...
      founder_object <- list(microbiome = microbiome_matrix,
...
#' @param taxa_assign Optional factor giving cluster assignment for each OTU in `otu_list`. When provided, `beta_otu` is built as a 90%/10% mixture of cluster-level and individual-level coefficients, inducing within-cluster correlation. If `NULL` (default), purely individual-level coefficients are used.
#' 
#' @examples
#' library(magrittr)
#' n_ind <- 10
...
calibrate_params_phenotypes <- function(X0,
  ...
  ## OTU effect coefficients
  if (!is.null(taxa_assign)) {
    ## mixture of cluster-level and individual-level coefficients (induces within-cluster correlation)
    taxa_assign <- as.factor(taxa_assign)
    beta_otu_clust <- mapvalues(taxa_assign, from = levels(taxa_assign),
                                to = rgamma(length(levels(taxa_assign)), shape = 1.4, scale = 3.8) *
                                     sample(c(1,-1), size = length(levels(taxa_assign)), replace = T)) %>%
      as.character() %>% as.numeric()
    beta_otu_ind <- rgamma(length(taxa_assign), shape = 1.4, scale = 3.8) *
      sample(c(1,-1), size = length(taxa_assign), replace = T)
    beta_otu <- 0.90 * beta_otu_clust + 0.10 * beta_otu_ind
    beta_otu_mix <- beta_otu
  } else {
...
#' @param taxonomy_table Optional taxonomy `data.frame` used to aggregate the microbiome at a given taxonomic rank before phenotype calibration. If `NULL` (default), phenotypes are calibrated on raw OTU-level abundances.
#' @param aggregate_rank Optional string giving the taxonomic rank (e.g. `"Family"`) or `"hclust"` to aggregate by genetic cluster, used jointly with `taxonomy_table`. Ignored if `taxonomy_table` is `NULL`.
...
holo_simu <- function(### Required parameters ###
  ...
  #####################
  if (!is.null(taxonomy_table) && !is.null(aggregate_rank)) {
    microbiome_pheno <- as.matrix(microbiome_gen)
    microbiome_pheno <- microbiome_pheno[otu_list, ] |> t() |> clrInv() |> t()
    
    if (aggregate_rank == "hclust") {
      id_group <- microbiome_pheno |> as_tibble() |>
        mutate(assignation = glue("Cluster_{n_clust[n_clust != 0]}")) |>
        group_by(assignation)
    } else {
      id_group <- dplyr::inner_join(
        microbiome_pheno |> as_tibble(rownames = "OTU"),
        taxonomy_table, by = "OTU"
      ) |>
        dplyr::select(!prevalence) |>
        dplyr::group_by(.data[[aggregate_rank]])
    }
    
    otu_list_calib    <- rownames(microbiome_pheno)
    B0_calib          <- microbiome_pheno |> t() |> clr() |> t()
    taxa_assign_calib <- group_indices(id_group)
  } else {
    otu_list_calib    <- otu_list
    B0_calib          <- microbiome_gen
    taxa_assign_calib <- NULL
  }
  
  params_phenotypes <- calibrate_params_phenotypes(X0 = get.geno(population,gen = 1),
                                                   B0 = B0_calib, #CLR abundances ,
                                                   h2 = h2,
                                                   b2 = b2,
                                                   otu_list = otu_list_calib,
                                                   Nqtl_y = Nqtl_y,
                                                   taxa_assign = taxa_assign_calib,
                                                   Notu_y = length(otu_list_calib))
  
  phenotypes_founder <- compute_phenotypes(X = get.geno(population,gen = 1), 
                                           B = B0_calib, #CLR abundances 
                                           Nqtl_y = Nqtl_y, 
                                           Notu_y = length(otu_list_calib), 
                                           otu_list = otu_list_calib,
