...
Then we've used a loop through iterations with different seeds and $\lambda$ values. This code could be optimized using purrr package. 
...
The useful informations of these simulations are the microbiomes, the pedigree and the ambient microbiome that could be extracted for each generation. We're focusing on G2 here.  
...
```{r, class.source = 'fold-show'}
#Could be functionalized and parallelized as done in previous section
